...
/** Tạo đơn mượn mới */
export const createLoanOrder = (data) => new LoanOrder(data).save();

/** Lấy tất cả (có thể truyền filter) */
export const getAllLoanOrders = (filter = {}) =>
  LoanOrder.find(filter).populate('bookId').populate('userId').exec();

/** Lấy chi tiết theo ID */
export const getLoanOrderById = (id) =>
  LoanOrder.findById(id).populate('bookId').populate('userId').exec();
...
/** Xóa đơn */
export const deleteLoanOrder = (id) => LoanOrder.findByIdAndDelete(id).exec();
...
export const confirmLoanOrder = (id) => updateLoanOrder(id, { status: 'confirmed' });
